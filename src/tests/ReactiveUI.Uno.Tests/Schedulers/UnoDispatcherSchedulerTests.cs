...
public class UnoDispatcherSchedulerTests
{
    ...
    /// <summary>The delay used when testing delayed scheduling.</summary>
    private const int ScheduledDelayMilliseconds = 100;

    /// <summary>The period used when testing periodic scheduling.</summary>
    private const int PeriodMilliseconds = 50;
    ...
    public void SetUp()
    {
        ...
        {
            Skip.Test(
                "Skipping scheduler tests because a CoreDispatcher test double cannot be created in this environment.");
        }
    }
    ...
    public async Task SchedulePeriodic_NullAction_ThrowsArgumentNullException() =>
        await Assert.That(
            () => _scheduler.SchedulePeriodic(State, TimeSpan.Zero, null!)).Throws<ArgumentNullException>();
