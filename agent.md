...
- **Main solution:** `src/ReactiveUI.Uno.slnx`
- **Benchmarks solution:** `Benchmarks/ReactiveUI.Uno.Benchmarks.sln`
...
```bash
git clone --recursive https://github.com/reactiveui/ReactiveUI.Uno.git
````
...
* Main file: `src/ReactiveUI.Uno.slnx`
...
dotnet build ReactiveUI.Uno.slnx -c Release
dotnet build ReactiveUI.Uno.slnx -c Release -warnaserror

dotnet clean ReactiveUI.Uno.slnx
```
...
# Run all tests
dotnet test --solution ReactiveUI.Uno.slnx -c Release

# Run tests for a specific project
dotnet test --project tests/ReactiveUI.Uno.Tests/ReactiveUI.Uno.Tests.csproj

# Run with code coverage (Microsoft Code Coverage)
dotnet test --solution ReactiveUI.Uno.slnx --coverage --coverage-output-format cobertura

# Detailed output (place BEFORE --)
dotnet test --solution ReactiveUI.Uno.slnx -- --output Detailed
dotnet test --solution ReactiveUI.Uno.slnx --coverage --coverage-output-format cobertura -- --report-trx --output Detailed

# List tests
dotnet test --project tests/ReactiveUI.Uno.Tests/ReactiveUI.Uno.Tests.csproj -- --list-tests

# Fail fast
dotnet test --solution ReactiveUI.Uno.slnx -- --fail-fast

# Limit parallelism if needed (even though repo defaults non-parallel)
dotnet test --solution ReactiveUI.Uno.slnx -- --maximum-parallel-tests 4
```
...
```powershell
# Single test
dotnet test --project tests/ReactiveUI.Uno.Tests/ReactiveUI.Uno.Tests.csproj -- --treenode-filter "/*/*/*/MyTestMethod"

# All tests in class
dotnet test --project tests/ReactiveUI.Uno.Tests/ReactiveUI.Uno.Tests.csproj -- --treenode-filter "/*/*/MyClassName/*"

# All tests in namespace
dotnet test --project tests/ReactiveUI.Uno.Tests/ReactiveUI.Uno.Tests.csproj -- --treenode-filter "/*/MyNamespace/*/*"

# Filter by property (e.g., Category)
dotnet test --solution ReactiveUI.Uno.slnx -- --treenode-filter "/*/*/*/*[Category=Integration]"
```
...
### Core Library (`src/ReactiveUI.Uno/`)
...
* Prefer existing patterns in:
  ...
  * `src/tests/ReactiveUI.Uno.Tests/`
* Use `ReactiveUI.Uno.Testing` utilities for reactive code
...
* Platform-specific code in `src/ReactiveUI.Uno/` core library
